...
const projectId = "e43f90cb83dc401db4ee59d2f88aa8a3";
//Your api secret in ifura.io
const projectSecret = "WFrJs4bRaykuJXE97bgbOLm4dLGqjXwbHAV1WTsuzVnjJBtdjPUxFA";
window.CONTRACT = {
  address: "0x1932c48b2bF8102Ba33B4A6B545C32236e342f34",
  network: "http://127.0.0.1:22001", // Point to Quorum RPC for Node 1
  explore: "http://127.0.0.1:22001", // Optional, depends on your setup
  // Your Contract ABI
  abi: [
    {
      inputs: [],
...
async function initializeContract() {
  window.web3 = new Web3(
    new Web3.providers.HttpProvider(window.CONTRACT.network)
  );
  window.contract = new window.web3.eth.Contract(
    window.CONTRACT.abi,
    window.CONTRACT.address // Use the contract address from the configuration
  );
}
...
async function connect() {
  try {
    ...
    // Populate the modal with the list of accounts
...
window.onload = async () => {
  ...
  if (window.userAddress && window.userAddress.length > 10) {
    ...
    $("#loginButton").hide();
    $("#userAddress")
      .html(`<i class="fa-solid fa-address-card mx-2 text-primary"></i>${window.userAddress}
...
function print_verification_info(result, is_verified) {
  ...
  if (!is_verified) {
    ...
    $("#file-hash").html(
      `<span class="text-info"><i class="fa-solid fa-hashtag"></i></span> ${window.hashedfile}`
    );
    $("#college-name").hide();
    ...
    $("#contract-address").html(
      `<span class="text-info"><i class="fa-solid fa-file-contract"></i> </span>${window.CONTRACT.address}`
    );
    $("#time-stamps").html(
    ...
    $("#student-address").html(
      `<span class="text-info"><i class="fa-solid fa-user"></i></span> ${result[4]}`
    );
...
function printUploadInfo(result) {
  ...
  $("#file-hash").html(
    `<i class="fa-solid fa-hashtag mx-1"></i> ${window.hashedfile}`
  );
  $("#contract-address").html(
    `<i class="fa-solid fa-file-contract mx-1"></i> ${result.to}`
  );
  ...
  $("#blockHash").html(
    `<i class="fa-solid fa-shield mx-1"></i> ${result.blockHash}`
  );
  $("#to-netowrk").html(
...
async function uploadFileToIpfs() {
  ...
  try {
    // Make post request to upload the file to the local IPFS node and get the CID
...
async function sendHash() {
  ...
  if (window.hashedfile.length > 4) {
    try {
      // Unlock the account before sending the transaction
      const unlocked = await web3.eth.personal.unlockAccount(
        window.userAddress,
        "", // Replace with the account's password
        600
      );
      if (!unlocked) {
      ...
      await window.contract.methods
        .addDocHash(window.hashedfile, CID, studentAddress)
        .send({
          from: window.userAddress,
          gas: 700000000, // Specify a higher gas limit
        ...
        .on("receipt", async function (receipt) {
          ...
          // Add hash, CID, receipt, and student address to MongoDB
          try {
            const response = await fetch(
              "http://localhost:4000/api/addDocument",
              {
                method: "POST",
                headers: {
            ...
            if (!response.ok) {
              throw new Error("Failed to add document to MongoDB");
            }
  ...
  if (window.hashedfile) {
    await window.contract.methods
      ...
      .on("receipt", async function (receipt) {
        ...
        try {
          const response = await fetch(
            `http://localhost:4000/api/deleteDocument`,
            {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
          ...
          if (!response.ok) {
            throw new Error("Failed to delete document from MongoDB");
          }
...
async function get_ChainID() {
  ...
  switch (a) {
    ...
    case 42:
      ...
      break;
    case 1337:
      window.chainID = "Ganache Local Network";
      break;
    default:
...
async function listen() {
  ...
  const fromBlock = Math.max(currentBlock - 999, 0); // Ensure fromBlock is not negative
  ...
  await window.contract.getPastEvents(
    "addHash",
    {
      ...
      fromBlock: window.web3.utils.toHex(fromBlock), // Convert to hexadecimal string
      toBlock: "latest",
    },
...
function printTransactions(data) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const image = document.createElement("img"); // Use <img> tag for displaying the image
    image.style = "width:100%;height: 100%;";

    // Use the local IPFS gateway
    image.src = `http://127.0.0.1:8080/ipfs/${data[i].returnValues[1]}`;
